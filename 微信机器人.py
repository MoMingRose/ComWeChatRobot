...
class ReceiveMsgBaseServer(socketserver.BaseRequestHandler):
        ...
        while True:
            try:
                ...
                if "订单状态提醒" in json_msg:
                    ReceiveMsgBaseServer.handle_article(msg)
                else:
    ...
    def handle_article(msg):
        ...
        if html is not None:
            url = ReceiveMsgBaseServer.url_compile.search(html)
            if url:
                print(f"========={url.group(1)}")
                wx.OpenBrowser(url.group(1))
...
def start_wechat_robot(pid, port=None, main_thread=True):
    """
    启动一个微信机器人实例，并开始服务与接收消息。

    参数:
        pid (int): 微信进程的PID。
        port (int): 微信机器人服务的端口号。如果为 None，则使用默认端口。
        main_thread (bool): 是否在主线程中启动 socket 服务器。默认为 True。
    返回:
        robot: 微信机器人实例。
    """
    robot = WeChatRobot(pid=pid)
    robot.StartService()
    userinfo = robot.GetSelfInfo()
    receive_status = robot.StartReceiveMessage(port=port)
    if receive_status == 0:
        print(userinfo.get("PhoneNumber"), "开始接收消息")
    if port is not None:
        start_socket_server(port=port, main_thread=main_thread)
    return robot

def run_wechat_service():
    """
    根据微信进程的数量运行对应数量的微信服务，并为每个微信服务分配不同的端口。

    """
    pid_list = get_wechat_pid_list()

    num_wechat = len(pid_list)

    if num_wechat == 0:
        print("请先登录为微信!")
        return

    ports = list(range(10800, 10800 + num_wechat))  # 分配端口
    for i, pid in enumerate(pid_list):
        port = ports[i]
        main_thread = i == num_wechat - 1  # 判断是否是最后一个元素
        robot = start_wechat_robot(pid, port=port, main_thread=main_thread)
