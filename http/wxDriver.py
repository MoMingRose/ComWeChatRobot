...
if __name__ == '__main__':
    ...
    start_listen(pids[1], port)

    print(post_wechat_http_api(APIS.WECHAT_MSG_START_HOOK, port, {"port": 10808}))

    start_socket_server(port=10808, main_thread=False)

    print(post_wechat_http_api(APIS.WECHAT_MSG_START_HOOK, port, {"port": 10809}))
    start_socket_server(port=10809)

    stop_listen(pids[0], port)
    start_listen(pids[1], port)
